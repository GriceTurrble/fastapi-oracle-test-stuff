...
class TestAuthorServiceInjectable:
    def test_wires_session_maker_via_base_class(self, mock_session_maker):
        ...
        touch: `AuthorServiceInjectable.__init__` is what FastAPI's
        `Depends()` actually calls in production, but router tests override
        it entirely (see `test/routers/authors/conftest.py`) and the tests
        above construct the base `AuthorService` directly. Calling it here
        is a plain Python call -- the `SessionMakerDep` annotation on its
        ...
        """
        injectable = author_service.AuthorServiceInjectable(
